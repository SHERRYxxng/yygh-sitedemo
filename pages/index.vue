...
export default {
  ...
  asyncData({ params, error }) {
    //加载医院列表（发起的同步请求，实现首页医院列表数据的同步渲染（服务端渲染））
    //1：第一页  10：每页10条   null：查询条件
    return hospApi.getPageList(1,10,null)
      .then(response => {
        return {
          list: response.data.pages.content,//当前页的医院列表
          pages: response.data.pages.totalPages//总页数
        }
      })
  },
  data() {
  ...
  methods:{
    ...
    init() {
      ...
      dictApi.findByDictCode('Hostype')
        .then(response => {
          ...
          // this.hostypeList = response.data.list
          // this.hostypeList.push({"name":"全部","value":""})
    ...
    show(hoscode) {
      ...
      //预约挂号页面（医院详情+预约须知）
      //后台系统中，打开任意一个vue，必须在router/index.js 路由文件中定义一个路由（path===》vue）
      //前台系统中，不需要定义路由，采用默认的动态路由
      //直接写路径即可，安装规则找到指定的vue
